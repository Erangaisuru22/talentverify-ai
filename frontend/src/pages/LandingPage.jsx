 ...
 * @description Public landing page for unauthenticated visitors.
 * Features the brand hero banner, value proposition highlights,
 * and a searchable public directory of active job listings with quick "Sign in to apply" triggers.
 */
 ...
 *
 * @param {Object} props
 * @param {Array} [props.jobs=[]] - List of public job postings.
 * @param {boolean} [props.loading=false] - Loading indicator state while fetching jobs.
 * @param {() => void} props.onSignIn - Trigger to open the login view.
 * @param {() => void} props.onCreateAccount - Trigger to open the registration view.
 * @param {(job: Object) => void} props.onApply - Triggered when visitor clicks apply on a job card.
 */
export default function LandingPage({ jobs = [], loading = false, onSignIn, onCreateAccount, onApply }) {
  // Search query state for filtering jobs
  const [query, setQuery] = useState('');

  // Memoized filtered jobs based on title, company, location, employment type, or required skills
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      ...
      {/* --- Hero Section: Value Proposition & AI Matching --- */}
      ...
      {/* --- Jobs Section: Search & Job Listing Grid --- */}
          ...
          {!loading && filteredJobs.map((job) => (
            <article key={job.id} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-slate-200/60">
              ...
              <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-slate-600">
                <span className="flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1.5">
