...
def deterministic_evaluation(job, candidate):
    ...
    project_ratio, project_matches = _ratio(required, [t for p in (projects or []) for t in (p.get("technologies") or []) if t])
    github = candidate.get("github") or candidate.get("github_profile") or {}
    github_ratio, github_matches = _ratio(required, (github.get("detected_skills") or []) if isinstance(github, dict) else [])
    portfolio = candidate.get("portfolio_evidence") or {}
    portfolio_matches = _names(portfolio.get("matched_skills"), "skill") if isinstance(portfolio, dict) else []
    try: required_years = float(job.get("experience") or 0)
    except (TypeError, ValueError): required_years = 0.0
    try: candidate_years = float((candidate.get("personal_info") or {}).get("experience_years") or 0)
    except (TypeError, ValueError): candidate_years = 0.0
    if not candidate_years:
        total_months = 0.0
        for x in (experiences or []):
            if isinstance(x, dict):
                try: total_months += float(x.get("duration_months") or 0)
    ...
    github_available = isinstance(github, dict) and github.get("verification_status") in {"verified", "success"}
    breakdown = {
    ...
    for requirement in (job.get("must_have_requirements") or []):
        if isinstance(requirement, dict):
            label = str(requirement.get("requirement") or "")
            kind = str(requirement.get("type") or "other")
        else:
            label = str(requirement or "")
            kind = "other"
