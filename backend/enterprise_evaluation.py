...
def validate_bands(value):
    ...
    if not bands or any(not isinstance(x.get("minimum"), (int, float)) or not x.get("label") for x in bands):
        raise ValueError("Recommendation bands require numeric minimum and label.")
    minimums = [float(x["minimum"]) for x in bands]
    if any(x < 0 or x > 100 for x in minimums) or len(set(minimums)) != len(minimums):
        raise ValueError("Recommendation band minimums must be unique values from 0 to 100.")
...
def evaluate_eligibility(candidate, requirements):
    ...
    for req in requirements or []:
        kind, target = req["type"], normalized(req["requirement"])
